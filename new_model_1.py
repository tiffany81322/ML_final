...
def create_anno_arr( filename ) :
	...
	for line in file :
		...
		q_type_string = line[ q_type_start:ans_type_start ].replace( 'question_type:"','' ).split()
		string_end = line.rindex('"')
		ans_type_string = line[ans_type_start:string_end].replace('"\tanswer_type:"','').split()
		vec_array = numpy.zeros(300)
		count = 0
		for word in q_type_string :
			count  = count+1
			try :
				vec_array = vec_array + dict[ word ]
			except KeyError :
		...
		ques_type_list.append( vec_array )
		if ans_type_string == 'number' :
			ans_type_list.append(1)
		else :
			ans_type_list.append(-1)

	return numpy.vstack( ques_type_list ),numpy.vstack( ans_type_list )
...
choice_train = numpy.vstack( create_choice_arr( 'final_project_pack/choices.train' ).values() )
...
train_ques_type,train_ans_type = create_anno_arr( 'final_project_pack/annotation.train' )
...
ques_nn.add(Dense(HIDDEN_SIZE, input_dim=WORD_VEC_LEN, init='uniform'))
ques_nn.add(Activation('tanh'))
ques_nn.add(Dense(HIDDEN_SIZE, init='uniform'))
ques_nn.add(Activation('tanh'))
ques_nn.add(Dropout(0.5))
ques_nn.add(Dense(HIDDEN_SIZE, init='uniform'))
ques_nn.add(Activation('tanh'))
ques_nn.add(Dropout(0.5))
ques_nn.add(Dense(HIDDEN_SIZE, init='uniform'))
ques_nn.add(Activation('tanh'))
ques_nn.add(Dropout(0.5))
ques_nn.add(Dense(WORD_VEC_LEN, init='uniform'))
ques_nn.add(Activation('tanh'))


sgd = SGD(lr=0.001, momentum=0.9, nesterov=True)
ques_nn.compile(loss="mean_squared_error", optimizer=sgd)

ques_nn.fit(X_train, train_ques_type, nb_epoch=10,verbose = 2)
score = ques_nn.evaluate(X_train, train_ques_type)

test_ques_value = X_test_dict.values()
test_choice_value = choice_test_dict.values()

train_hidden = ques_nn.predict( X_train )
#train_hidden = train_ques_type
test_hidden = ques_nn.predict( numpy.vstack( X_test_dict.values() ) )
...
for i in xrange( len( X_train ) ) :
	NN_in.append( numpy.hstack( [X_train[ i ],choice_train[ i ],train_hidden[i] ] ) )
NN_in = numpy.vstack( NN_in )

NN_test = []

for i in xrange( len( X_test_dict ) ) :
	NN_test.append( numpy.hstack( [ test_ques_value[ i ],test_choice_value[ i ],test_hidden[i] ] ) )
...
model.add(Dense(HIDDEN_SIZE, input_dim=7*WORD_VEC_LEN, init='uniform'))
...
model.fit(NN_in, sol_class, nb_epoch=15,verbose = 2)
